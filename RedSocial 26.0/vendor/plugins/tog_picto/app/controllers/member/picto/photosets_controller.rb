class Member::Picto::PhotosetsController < Member::BaseController
  ...
  def create
    ...
    @photoset.id_referencia = params[:photoset][:id_referencia]
    @photoset.owner = current_user
    if @photoset.id_referencia == 'Para mi'
      @photoset.id_referencia = current_user
      @photoset.tipo = 'Usuario'
    else
        @photoset.tipo = 'Grupo'
    end
    @photoset.save!
...
=begin
  Método para destruir álbum de fotos
  1º)buscamos el álbum de fotos del que es propietario el usuario actua,
  2º)eliminamos todas las fotos pertenecientes a ese álbum
  3º)eliminamos el álbum de fotos.
=end  
  def destroy
    @photoset = current_user.owned_photosets.find(params[:id])
