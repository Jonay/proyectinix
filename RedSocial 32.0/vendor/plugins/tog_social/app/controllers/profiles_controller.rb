class ProfilesController < ApplicationController
  ...
  def photosets
    id = params[:id]
    @photosets = Picto::Photoset.find(:all, :conditions => {:id_referencia => id, :tipo => 'Usuario'})
  ...
  def responsibles
    id = params[:id] #pasamos la id del perfil del usuario
    @responsibles = Responsible.find(:all, :conditions => {:profile_id => id})
    
    respond_to do |format|
      format.html # responsibles.html.erb
      format.xml  { render :xml => @responsibles.to_xml }
    end
  end  
