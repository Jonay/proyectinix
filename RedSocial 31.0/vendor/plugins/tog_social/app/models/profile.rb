class Profile < ActiveRecord::Base
  ...
  #validates_presence_of  :activity, :message => "Introduzca nombre de su empresa"
  ...
  has_attached_file :pdf, {
    :url => "/system/:class/:attachment/:id/:style_:basename.:extension",
    :path => "/system/:rails_root/:public/:profile/:pdfs/:id/:style/:basename.:extension"
  }.merge(Tog::Plugins.storage_options)
  
  validates_attachment_content_type :pdf, :content_type => [ 'application/pdf', 'application/msword', 
                                                             'application/vnd.openxmlformats-officedocument.wordprocessingml.document' ],
                                          :if => :pdf_attached?,
                                          :message => 'Asegurese que el fichero tiene la extensión .pdf o .doc'
  ...
  def confirm_friend(friend)
    Friendship.make_friends(friend, me)
    return true
  end
  ...
  def new_responsible
    ...
    @responsible = @profile.responsibles.build
  end
  
  def number_of_responsibles
    responsibles.length
  end
  
   def create_responsible
    @profile = Profile.find(params[:id])
    @responsible = @profile.responsibles.build(params[:actor])
    if @responsible.save
      flash[:notice] = "Your responsible was successfully added."
