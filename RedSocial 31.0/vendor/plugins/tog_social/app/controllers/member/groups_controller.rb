class Member::GroupsController < Member::BaseController
  ...
  before_filter :find_group, :except => [:index, :new, :create, :confirm_moderator]
      ...
      if @group.invited_members.include? u
        ...
        if current_user.profile.is_related_to?(u.profile)
          ...
          #url para confirmar pasándole usuario actual y el grupo
                           ...
                           :reject_url => member_group_reject_invitation_url(current_user.profile, @group.id))
          ).dispatch!
...
=begin
  -- Método para rechazar invitaciones --
  1º) Si el usuario rechaza la invitacion lo eliminamos como miembro invitado al grupo y
      envía un mensaje al usuario que lo invitó.
  2º) Si el usuario no rechazar es que no está invitado.
=end  
  def reject_invitation
    ...
    if(@group.leave(current_user))
      Message.new(
          :from => current_user,
          :to => u,
          :subject => I18n.t("tog_social.groups.member.mail.reject_invitation.subject", :user_name => current_user.profile.full_name),
          :content => I18n.t("tog_social.groups.member.mail.reject_invitation.content", :u_name => u.profile.full_name, 
                           :user_name   => current_user.profile.full_name, 
                           :group_name => @group.name, 
                           :user_profile_url => profile_url(current_user.profile))
      ).dispatch!
...
=end
  def add_moderator
    ...
    group= @group
    if @group.moderators.include? u
      flash[:error] = I18n.t("tog_social.groups.member.moderator.already_moderator")
    else
      if current_user.profile.is_related_to?(u.profile) 
        Message.new(
          :from => current_user,
...
=begin
  ...
  2º si el usuario actual es miembro del clúster lo eliminamos, luego creamos como nuevo miembro 
...
=begin
  ...
  1º almacenamos el creador del grupo en la variable admin, y si el moderador a eliminar es el usuario
     indicamos con un mensaje que no se puede elimnar al creador del grupo. 
     De tal forma que nunca podrán eliminar al creador del grupo.
...
=end
  def remove_moderator
    ...
    if u.id == admin
      flash[:error] = I18n.t("tog_social.groups.site.is_the_main_moderator")
    else
