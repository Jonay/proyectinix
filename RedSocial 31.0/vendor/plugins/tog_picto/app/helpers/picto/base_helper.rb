module Picto
  module BaseHelper
    ...
    def edit_text_navigation_links(photo, options={}, html_options={:class => "nav-text-links"})
      default_options = {
        :prev_text => "<< Previous",
        :next_text => "Next >>",
        :separator => "|"
      ...
      content_tag :div, html_options do
        <<-eos
        #{link_to default_options[:prev_text], edit_member_picto_photo_url(photo.higher_item), :class => "nav-text-prev" unless photo.higher_item.nil?}
        #{default_options[:separator]}
        #{link_to default_options[:next_text], edit_member_picto_photo_url(photo.lower_item), :class => "nav-text-next" unless photo.lower_item.nil?}
        eos
      end
    end
    
    def edit_image_navigation_links(photo, options={}, html_options={:class => "nav-image-links"})
      default_options = {
        :prev_text => "<< Previous",
        :next_text => "Next >>",
      }.merge(options)
      ...
      content_tag :div, html_options do
        <<-eos
        #{ link_to image_tag(photo.higher_item.image.url(:tiny)), edit_member_picto_photo_path(photo.higher_item), :title => default_options[:prev_text], :class => "nav-image-prev" unless photo.higher_item.nil? }
        #{ link_to image_tag(photo.lower_item.image.url(:tiny)), edit_member_picto_photo_path(photo.lower_item),  :title => default_options[:next_text], :class => "nav-image-next" unless photo.lower_item.nil? }
        eos
      end
    end
