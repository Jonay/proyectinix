...
=begin
  ...
  2º si ese usuario está relacionado con el usuario actual, y en el caso de que sea miembro del grupo
     eliminamos ese miembro del grupo llamando a @group.leave y luego creamos uno nuevo 
     con los parámetros @group, indicando que es moderator = true y estado activo y mostramos 
     un mensaje indicando que el moderador se añadió.
...
=end
  def add_moderator
    ...
    if current_user.profile.is_related_to?(u.profile) 
      if (@group.membership_of(u)) || (@group.members.include? u)
        @group.leave(u)
      end
...
=end
  def remove_moderator
    if @group.moderators.include?(current_user) && @group.moderators.size == 1
      ...
      @membership = @group.memberships.find(params[:user_id]).destroy
