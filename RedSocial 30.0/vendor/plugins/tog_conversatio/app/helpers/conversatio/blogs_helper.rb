  ...
  module BlogsHelper
    ...
    def archives_menu
      ...
      for year in publication_years
        html << "#{link_to("#{year}", yearly_archives_conversatio_blog_posts_path(@blog, year))}"
        if @year
          html << "<ul>"
          for m in 1..12
            html << "#{link_to(I18n.t('date.abbr_month_names')[m], monthly_archives_conversatio_blog_posts_path(@blog, year, m))}, "
          end
          html << "</ul>"
        end
      end
